...
help_text = """
Привет! Вот вся необходимая информация о КПТ-Боте.
...
*Как начать?*
Просто напиши что-нибудь если хочешь сорваться и покурить, и мы начнем вместе работать над твоими привычками и эмоциональным состоянием. Бот доступен 24/7, так что помощь всегда под рукой.
"""
...
Помни, что ты не один, и всегда можешь обратиться ко мне за помощью. 🌿

Я тебе буде рассказывать что какие улучшения у тебя уже произошли с момента отказа от курения.
"""
STOP_SMOKING_ALREADY_STARTED = """
Ты уже решил бросить курить! 🎉

С твоим прогрессом всё в порядке, и я уже присылаю тебе сообщения поддержки с момента отказа от курения:
📅 Дата начала: *{time}*. 
🕒 Прошло времени: *{execute_time}*.

Продолжай, ты на правильном пути! 💪 Если нужно что-то отменить или получить дополнительную помощь, просто скажи.
"""
STOP_SMOKING_CANCELLED = """
Ты решил отменить поддержку сообщений. 📭

С момента отказа от курения прошло: {days} дней, {hours} часов и {minutes} минут. Ты уже сделал большой шаг к свободе от никотина! 🚭

Если передумаешь, просто напиши, и я снова помогу тебе в этом пути.
"""
...
Теперь, когда ты на шаг ближе к свободе от курения, не забывай — ты всегда можешь *написать* мне или *записать голосовое*, когда захочешь курить. 

Ксатати просто вызови команду `/stop_smoking`, и мы вместе начнем новый этап — твою жизнь без никотина! 🚭
